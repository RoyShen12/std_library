...
(function (glb) {
  const util = require('./inner/_util')
  const _Node = require('./inner/_node')
  ...
  class list {
     ...
     * @param {list | ArrayLike | any} objToTest
     * @returns {objToTest is list}
     */
     ...
     * @param {_Node} node
     * @param {(index: number, p: _Node) => number} callback
     */
    ...
    constructor(elem) {
      ...
      return new Proxy(this, {
        get: function (target, key, recv) {
          // key is of numbers
          if (typeof key !== 'symbol' && !isNaN(key) && key !== '' && key !== ' ') {
    ...
    /**
     * forward iterator
     * @param {(index: number, p: _Node) => number} callback - callback function to handle the index and node while iterating, return -1 to break the iterator loop
     */
    itr(callback) {
     ...
     * @param {(itemData: any, index: number, thisList: list) => void} callback
     */
     ...
     * @param {(itemDataFirst: any, itemDataSecond: any, indexFirst: number, indexSecond: number, thisList: list) => void} callback
     */
     ...
     * @param {(itemData: any, index: number, thisList: list) => boolean} callback
     */
    every(callback) {
      if (Object.prototype.toString.call(callback) !== '[object Function]') {
     ...
     * @param {(itemData: any, index: number, thisList: list) => any} callback
     */
    map(callback) {
      if (Object.prototype.toString.call(callback) !== '[object Function]') {
     ...
     * @param {number} subListLength
     */
    sub_list(startIndex, subListLength) {
      if (this.empty()) {
        return new list()
      }
     ...
     * @param {(a: any, b: any) => number} cmpFunc
     */
    sort(cmpFunc = util.defaultMinus) {
      ...
      function quick_sort(pl, ph) {
         ...
         * @param {_Node} pstLow
         * @param {_Node} pstHigh
         * @returns {() => ReturnType<quick_sort_inner>[]}
         */
        ...
        /**
         * @param {typeof quick_sort_inner} func
         * @param {[_Node, _Node]} arg
         */
    ...
    /**
     * @param {(item: any) => boolean} equFunc
     */
    find(equFunc) {
    ...
    /**
     * @param {list} anotherListRef
     * @param {number} position [-1, length-1], concat after position
     * @returns {this}
     */
